...
export function loopcraftSystemPrompt(): string {
  return [
    ...
    "Keep each build stage small enough to finish inside its budgets, and in each",
    "build action's params set a generous `wall_timeout` (seconds) AND `max_turns`",
    "sized to the stage — e.g. `params: { wall_timeout: 1800, max_turns: 80 }` for a",
    "substantial stage. A stage needing more than that is too big — split it. Each",
    "build stage should WRITE its code and do only a QUICK sanity check; the paired",
    "test action does the real verification, so a build agent must not spend its",
    "turns exhaustively booting and debugging the server in-band.",
    "",
    "# Applying edits",
