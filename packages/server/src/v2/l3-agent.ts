...
export function loopcraftSystemPrompt(): string {
  return [
    ...
    "Keep each build stage small enough to finish inside its time budget, and set a",
    "generous `wall_timeout` (seconds) in each build action's params sized to the",
    "stage — e.g. `params: { wall_timeout: 1800 }` for a substantial stage. A stage",
    "that would need more than ~40 minutes of build time is too big — split it.",
    "",
    "# Applying edits",
