...
export function loopcraftSystemPrompt(): string {
  return [
    ...
    "stage. A build stage WRITES its code (and its tests); the paired test node runs the",
    "suite (`bun test`) to verify runtime behavior. Build agents must NOT boot servers to",
    "smoke-test in-band — that is the test node's job and it burns the build's turn budget.",
    "",
    "# Applying edits",
